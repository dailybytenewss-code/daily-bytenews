...
export default function NewArticlePage() {
  ...
  const handleCategoryChange = (label: string) => {
    ...
    setForm((prev) => ({ ...prev, category: cat.label, categorySlug: cat.slug, categoryColor: cat.color }));
  };
      ...
      <div className="max-w-4xl">
        ...
        <div className="flex items-center gap-4 mb-6 flex-wrap">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Fill in all fields and publish</p>
          </div>
          <div className="ml-auto flex items-center gap-3">
            ...
            >
              {saved ? <><CheckIcon className="w-4 h-4" />Saved!</> : saving ? 'Saving...' : status === 'draft' ? 'Save Draft' : status === 'scheduled' ? 'Schedule' : 'Publish'}
            </button>
          </div>
        </div>
        ...
        <form id="article-form" onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 space-y-4">
            ...
            <div>
              ...
              <input required type="text" value={form.title} onChange={(e) => handleTitleChange(e.target.value)}
                placeholder="Enter article headline..." className={inputCls} />
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <input required type="text" value={form.slug} onChange={(e) => set('slug', slugify(e.target.value))}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 space-y-4">
            ...
            <div>
              ...
              <input type="text" value={form.tags} onChange={(e) => set('tags', e.target.value)}
                placeholder="AI, OpenAI, ChatGPT (comma-separated)" className={inputCls} />
            </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input required type="text" value={form.author} onChange={(e) => set('author', e.target.value)}
                  placeholder="Ananya Krishnan" className={inputCls} />
              </div>
              <div>
                <label className={labelCls}>Avatar URL</label>
                <div className="flex gap-2">
                  <input type="url" value={form.authorAvatar} onChange={(e) => set('authorAvatar', e.target.value)}
                    placeholder="https://..." className={`${inputCls} flex-1`} />
                  <button type="button" onClick={() => openMediaPicker('cover')}
            ...
            <div>
              ...
              <textarea rows={2} value={form.authorBio} onChange={(e) => set('authorBio', e.target.value)}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input required type="url" value={form.image} onChange={(e) => set('image', e.target.value)}
                  placeholder="https://..." className={inputCls} />
              </div>
              <div>
                <label className={labelCls}>Alt Text <span className="text-red-500">*</span></label>
                <input required type="text" value={form.imageAlt} onChange={(e) => set('imageAlt', e.target.value)}
                  placeholder="Descriptive text for accessibility" className={inputCls} />
              </div>
            </div>
            <div>
              <label className={labelCls}>Caption <span className="text-xs text-gray-400 font-normal">(optional)</span></label>
              <input type="text" value={form.imageCaption} onChange={(e) => set('imageCaption', e.target.value)}
                placeholder="Image caption shown below photo" className={inputCls} />
            </div>
            {form.image && (
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
            ...
            >
              ...
              {seoOpen ? <ChevronUpIcon className="w-4 h-4 text-gray-400" /> : <ChevronDownIcon className="w-4 h-4 text-gray-400" />}
            </button>
            {seoOpen && (
              <div className="px-5 pb-5 space-y-4 border-t border-gray-200 dark:border-gray-800 pt-4">
                <div>
                  ...
                  <input type="text" value={form.metaTitle} onChange={(e) => set('metaTitle', e.target.value)}
                    maxLength={60} placeholder="SEO title (defaults to article title)" className={inputCls} />
                </div>
                <div>
                  <label className={labelCls}>
                    Meta Description
                  ...
                  <textarea rows={3} value={form.metaDescription} onChange={(e) => set('metaDescription', e.target.value)}
                    maxLength={160} placeholder="Short description for search engines (defaults to excerpt)"
                    className={`${inputCls} resize-none`} />
                </div>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input type="url" value={form.ogImage} onChange={(e) => set('ogImage', e.target.value)}
                        placeholder="https://... (defaults to cover image)" className={`${inputCls} flex-1`} />
                      <button type="button" onClick={() => { setMediaTarget('cover'); setShowMediaPicker(true); }}
                        className="px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:border-blue-400 transition-colors flex-shrink-0">
                        <PhotoIcon className="w-4 h-4" />
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <input type="url" value={form.canonical} onChange={(e) => set('canonical', e.target.value)}
                      placeholder="https://dailybytenews.in/article?slug=..." className={inputCls} />
                  </div>
                </div>
                {/* Preview */}
                {(form.metaTitle || form.title) && (
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                    ...
                    <p className="text-blue-700 dark:text-blue-400 text-sm font-medium line-clamp-1">{form.metaTitle || form.title || 'Article Title'}</p>
                    <p className="text-green-700 dark:text-green-500 text-xs mt-0.5">dailybytenews.in › article</p>
                    <p className="text-gray-600 dark:text-gray-400 text-xs mt-1 line-clamp-2">{form.metaDescription || form.excerpt || 'Meta description will appear here.'}</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex justify-between items-center pb-4">
            <Link href="/admin/articles" className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
              ← Back to Articles
            </Link>
            <div className="flex items-center gap-3">
              ...
              <button type="submit" disabled={saving || saved}
                ...
                {saved ? <><CheckIcon className="w-4 h-4" />Saved!</> : saving ? 'Publishing...' : status === 'draft' ? 'Save Draft' : status === 'scheduled' ? 'Schedule Article' : 'Publish Article'}
              </button>
            </div>
          </div>
        </form>
