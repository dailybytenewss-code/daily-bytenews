  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Articles', value: totalArticles, icon: DocumentTextIcon, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20' },
          { label: 'Categories', value: categoryCount, icon: TagIcon, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20' },
          { label: 'Trending', value: trendingCount, icon: ArrowTrendingUpIcon, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-900/20' },
          { label: 'Featured', value: featuredCount, icon: StarIcon, color: 'text-green-600 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-900/20' },
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-800">
            <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Articles by Category</h3>
            <Link href="/admin/categories" className="text-xs text-blue-600 dark:text-blue-400 hover:underline">
              Manage
            </Link>
          </div>
          <div className="p-5 space-y-3">
            {articlesByCategory.map((cat) => (
              <div key={cat.slug} className="flex items-center gap-3">
                <span className={`text-xs font-medium px-2 py-0.5 rounded border ${colorMap[cat.color] || colorMap.blue}`}>
                  {cat.name}
                </span>
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400 w-4 text-right">{cat.count}</span>
              </div>
            ))}
          </div>
        </div>
